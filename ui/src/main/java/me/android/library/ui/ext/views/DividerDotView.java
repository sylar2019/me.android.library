...
public class DividerDotView extends View {
    ...
    private void init(Context cx, AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            TypedArray a = cx.obtainStyledAttributes(attrs, R.styleable.DividerDotView);
            dotColor = a.getColor(R.styleable.DividerDotView_dot_line_color, DOT_COLOR);
            dotRadius = a.getDimensionPixelSize(R.styleable.DividerDotView_dot_line_dot_radius, DOT_RADIUS);
            dashGap = a.getDimensionPixelSize(R.styleable.DividerDotView_android_dashGap, DASH_GAP);
            orientation = a.getInt(R.styleable.DividerDotView_android_orientation, HORIZONTAL);
            a.recycle();
        }
